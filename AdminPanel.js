...
export default function AdminPanel({
  ...
  const handleTribeNameSave = (tribeId) => {
    ...
    if (val && val.trim()) {
      onUpdateTribeName(tribeId, val.trim());
    }
  ...
  const handleAssignToggle = (tribeId, survivorId) => {
    ...
    const totalAssigned = tribes.filter(t => t.id === tribeId)[0].survivors.length;
    // Each tribe gets max 3
    if (!already && totalAssigned >= 3) {
      alert('Each tribe can only have 3 survivors.');
      return;
    }
    // Check if already assigned to another tribe
  ...
  // --- Survivor name editing ---
  const handleSurvivorNameChange = (id, val) => {
    setEditingSurvivorNames(prev => ({ ...prev, [id]: val }));
  };
    ...
    <div className="admin-overlay">
      <div className="admin-panel">
        ...
        <div className="admin-panel__body">
          ...
          {activeTab === 'assign' && (
            <div>
              <p className="admin-hint">Select up to 3 survivors per tribe. A survivor can only belong to one tribe.</p>
              {tribes.map(tribe => (
                    ...
                    {survivors.map(s => {
                      ...
                      return (
                        <button
                          ...
                          title={isAssignedElsewhere ? 'Already drafted' : ''}
                        >
                          {s.name}
                        </button>
                      );
          ...
          {activeTab === 'eliminate' && (
            <div>
              <p className="admin-hint">Mark survivors as eliminated in order. The next elimination will be #{eliminatedCount + 1}.</p>
                  ...
                  .map(s => (
                    <div key={s.id} className={`elim-row ${s.eliminated ? 'elim-row--out' : ''}`}>
                      <div className="elim-row__info">
                        ...
                        {s.eliminated && (
                          <span className="elim-row__order">Boot #{s.eliminationOrder}</span>
                        )}
                      </div>
                      <div className="elim-row__actions">
                        {s.eliminated ? (
                          <button className="btn-reinstate" onClick={() => onReinstateSurvivor(s.id)}>
                            Reinstate
                          </button>
                        ) : (
                          <button className="btn-eliminate" onClick={() => handleEliminate(s.id)}>
                            Eliminate
                          </button>
                        )}
                      </div>
              ...
              {tribes.map(tribe => (
                <div key={tribe.id} className="admin-section">
                  ...
                  <div className="admin-field">
                    ...
                    <div className="admin-inline">
                      {tribe.icon && (
                        <img src={tribe.icon} alt="icon" className="admin-icon-preview" />
                      )}
                      <label className="btn-upload">
                        ...
                        <input
                          ...
                          accept="image/*"
                          style={{ display: 'none' }}
                          onChange={e => handleIconUpload(tribe.id, e)}
                        />
                      </label>
                    </div>
                  </div>
          ...
          {activeTab === 'cast' && (
            <div>
              <p className="admin-hint">Edit the names of the 24 survivors to match the actual Survivor 50 cast.</p>
                ...
                {survivors.map(s => (
                  <div key={s.id} className="cast-row">
                    ...
                    <input
                      ...
                      onChange={e => handleSurvivorNameChange(s.id, e.target.value)}
